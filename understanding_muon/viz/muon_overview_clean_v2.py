...
class MuonOverview3D(ThreeDScene):
    def construct(self):
        ...
        self.play(FadeIn(surface, shift=0.5 * IN), run_time=0.6)
        ...
        start_theta = self.camera.get_theta()
        self.move_camera(
            theta=start_theta + TAU,  # 360°
            run_time=1.8,  # pick 1.5–2.5s to taste
            rate_func=linear,
        )
...
class MuonOverview3DSimple(ThreeDScene):
    def construct(self):
        ...
        self.play(LaggedStart(*[FadeIn(c) for c in contours_3d], lag_ratio=0.1), run_time=1.0)
        self.wait(0.2)
        ...
        start_theta = self.camera.get_theta()
        self.move_camera(theta=start_theta + TAU, run_time=2.5, rate_func=linear)

        # try to save a frame
        frame = self.renderer.get_frame()
        fw = self.renderer.file_writer
        save_image = getattr(fw, "save_image", getattr(fw, "save_final_image"))
        try:
            save_image(frame)
        except Exception:
            import numpy as np
        ...
        # Top-down camera
        self.move_camera(phi=0 * DEGREES, theta=-90 * DEGREES, gamma=0, run_time=1.0)
        self.wait(0.2)
